...
User will provide a work order description and a report on important failure codes relavant to
the work order description. 

Work order description: {taskdescription}

Failure codes discussion: {failurecodediscussion}

Your task is to assign the work order the appropriate failure code.

"""

# most important --> relevant

topktemplate = """Generate top {K} failure codes. In case of tie, pick the failure code for which work order
description has the best match with failure description. Only provide failure code in list."""
...
for mdl in LLMsets:
    ...
    failurecodeList = list(df['0'])

    #@ray.remote
    def run_one(assetdescription, assetlocation, taskdescription, failurecodediscussion, K=5):
        """
        pass the topic
        """
        Sprompt = SystemPrompt.format(assetdescription=assetdescription,
                                    ...
                                    failurecodediscussion=failurecodediscussion)

        qpromt = topktemplate.format(K=K)
