...
for mdl in LLMsets:
    ...
    def run_one(assetdescription, assetlocation, taskdescription):
        ...
        qpromt = """Extract relevant failure codes from the given document
        along with their rank. Provide them as a single list in python code
        block. Each element of list is a tuple with failure code being the first entry and 
        the numeric rank being the second entry. Do not generate 
        any additional explanation and justification.
        ...
        print (json_res.strip().split('\n\n')[0])
        return [json_res.strip().split('\n\n')[0]]
