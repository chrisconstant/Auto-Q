...
class EchoParser(nn.Module):
    def __init__(self, tokenizer, templates, max_length=None):
        ...
        if isinstance(tokenizer, str):
            self.tokenizer = AutoTokenizer.from_pretrained(
                tokenizer, add_bos_token=False, add_eos_token=False
            )
        ...
        self.template_pieces = {
            k: self._parse_template(template) for k, template in templates.items()
        }
    ...
    def _tokenize_piece(self, x, template_piece):
        if isinstance(template_piece, str):
            ...
            tokenized_piece = (
                tokenized_piece[: self.max_length]
                if self.max_length is not None
                else tokenized_piece
            )
            attention_mask = torch.ones(len(tokenized_piece), dtype=torch.long)
            embed_mask = torch.full(
                (len(tokenized_piece),), embed_mask_value, dtype=torch.long
            )
            return {
            ...
            template_piece = (
                template_piece[: self.max_length]
                if self.max_length is not None
                else template_piece
            )
    ...
    def _tokenize_from_pieces(self, x, template_pieces):
        token_pieces = [
            self._tokenize_piece(x, template_piece)
            for template_piece in template_pieces
        ]
        tokens = {k: torch.cat([z[k] for z in token_pieces]) for k in token_pieces[0]}
        return tokens
    ...
    def tokenize(self, xs):
        ...
        tokenized = [
            self._tokenize_from_pieces(x[1], self.template_pieces[x[0]]) for x in xs
        ]
            ...
            if len(x["input_ids"]) < max_tokenized_length:
                x["input_ids"] = torch.cat(
                    [
                        ...
                        torch.tensor(
                            ...
                            * (max_tokenized_length - len(x["input_ids"])),
                            dtype=torch.long,
                        ),
                    ]
                )
                x["attention_mask"] = torch.cat(
                    [
                        x["attention_mask"],
                        torch.zeros(
                            max_tokenized_length - len(x["attention_mask"]),
                            dtype=torch.long,
                        ),
                    ]
                )
                x["embed_mask"] = torch.cat(
                    [
                        x["embed_mask"],
                        torch.zeros(
                            max_tokenized_length - len(x["embed_mask"]),
                            dtype=torch.long,
                        ),
                    ]
                )
        tokenized = {k: torch.stack([z[k] for z in tokenized]) for k in tokenized[0]}
        return tokenized
...
class EchoPooling(nn.Module):
    ...
    def forward(self, xs):
        ...
        if self.strategy == "mean":
            pooled = torch.sum(
                token_embeddings * embed_mask.unsqueeze(-1), dim=1
            ) / torch.sum(embed_mask, dim=1).unsqueeze(-1)
            ...
            i = torch.arange(token_embeddings.shape[0]).reshape(
