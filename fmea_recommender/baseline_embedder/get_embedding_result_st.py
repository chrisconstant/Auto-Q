...
# Load data
train_df = pd.read_csv("./data/all_ids_deduplicated_train_short_desc_to_failure_locations.csv")
test_df = pd.read_csv("./data/all_ids_deduplicated_test_short_desc_to_failure_locations.csv")
val_df = pd.read_csv("./data/all_ids_deduplicated_val_short_desc_to_failure_locations.csv")
...
def process_cos_sim(cos_sim, prefixmatch):
    ...
    df_values = pd.DataFrame(top_values_list, columns=[f"Top Value {i+1}" for i in range(3)])
    df_indices = pd.DataFrame(top_indices_list, columns=[f"Top Index {i+1}" for i in range(3)])
