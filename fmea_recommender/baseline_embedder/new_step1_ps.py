...
def build_all_prompts(
    ...
    for cnt, bm in enumerate(best_k_matches):
        ...
        if permutation:
            gold_sds, gold_flss = ps.shuffle_examples(gold_sds, gold_flss, permutation)
        val_sd = sds[cnt]
        prompt = prompt_method(gold_sds, gold_flss, val_sd)
        all_prompts.append(prompt)
    return all_prompts

def get_retreval_result(
    sds, best_k_matches, dynamic_shots_cfg
):
    """Build and gather all prompts into a list."""
    all_results = []
    for cnt, bm in enumerate(best_k_matches):
        ...
        for idx in bm:  # Prepare relevant examples.
            ...
            # Sort Examples!!!
            gold_fls = sort_components(gold_fls)  # Sort examples alphabetically
            gold_sds.extend(gold_sd)
            gold_flss.extend(ast.literal_eval(gold_fls))

        all_results.append(gold_flss)
    return all_results
...
def generate_components(
    ...
    best_k_matches = ps.get_related_examples(
        sds, dynamic_shots_cfg, max_num_egs_in_prompt
    )
...
def generate_components_baseline(
    sds,
    dynamic_shots_cfg,
    max_num_egs_in_prompt=4,
):
    ...
    all_failure_locations = get_retreval_result(
        sds, best_k_matches, dynamic_shots_cfg)

    return all_failure_locations, all_failure_locations
