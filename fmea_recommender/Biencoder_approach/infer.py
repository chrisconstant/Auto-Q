...
def gen_retrieval_result(
    ...
    for user_emb in user_embeddings:
        scores = (
            torch.softmax(torch.matmul(user_emb, item_embeddings.T), -1)
            .squeeze()
            .tolist()
        )
...
def parse_args():
    ...
    parser.add_argument("--answer_file", type=str, help="")
    parser.add_argument("--topk", type=int, default=10, help="")
    parser.add_argument("--seed", type=int, default=2023, help="")
    parser.add_argument("--query_max_len", type=int, default=512, help="")
    parser.add_argument("--passage_max_len", type=int, default=128, help="")
    parser.add_argument("--per_device_eval_batch_size", type=int, default=128, help="")
    parser.add_argument(
        "--sentence_pooling_method",
        type=str,
        default="cls",
        help="the pooling method, should be cls, mean or last",
        choices=["cls", "mean", "last"],
    )
    parser.add_argument("--normlized", action="store_true", help="")
    parser.add_argument("--has_template", action="store_true", help="")
    parser.add_argument("--peft_model_name", type=str, default=None, help="")
    parser.add_argument(
        "--torch_dtype",
        type=str,
        default=None,
...
if __name__ == "__main__":
    ...
    run_model_embedding(
        ...
        max_seq_len=args.passage_max_len,
        batch_size=args.per_device_eval_batch_size,
        prompt_path=item_embedding_prompt_path,
        emb_path=item_embedding_path,
        accelerator=accelerator,
        args=args,
        qorp="passage",
    )
    ...
    run_model_embedding(
        args.model_path_or_name,
        max_seq_len=args.query_max_len,
        batch_size=args.per_device_eval_batch_size,
        prompt_path=args.user_embedding_prompt_path,
        emb_path=user_embedding_path,
        accelerator=accelerator,
        args=args,
        qorp="query",
    )
