...
import random
import json
from template import description2label_template, example2label_template
...
def parse_args():
    ...
    parser.add_argument("--out_example2label", default='', type=str, help="")
...
def gen_description2label(itemid2label, itemid2description, args):
    ...
    with open(os.path.join(args.out_directory, args.out_description2label), "w") as f:
        ...
        for index in range(len(itemid2label)):
            ...
            truncated_query = (
                tokenizer.convert_tokens_to_string(tokens).strip().strip(",")
            )
...
def gen_example2label(itemid2label, itemid2examples, args):
    """_summary_
    ...
    with open(os.path.join(args.out_directory, args.out_example2label), "w") as f:

        for index in range(len(itemid2label)):

            # there are multiple examples
            for exp in itemid2examples[index]:

                if random.random() < 0.5:
                    template = "{}"
                else:
                    template = random.choice(example2label_template)
...
if __name__ == "__main__":
    ...
    gen_example2label(itemid2label, itemid2examples, args)
