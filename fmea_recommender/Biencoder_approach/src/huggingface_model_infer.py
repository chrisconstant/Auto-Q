...
class EmbDataset(Dataset):
    def __init__(
        self, test_dataset, tokenizer, max_seq_len, has_template, qorp
    ) -> None:
        super().__init__()
...
class InferCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        if not self.has_template:
            ...
            eos = torch.full(
                (t_batch, 1), self.tokenizer.eos_token_id, dtype=torch.long
            )
            eos_attn = torch.ones((t_batch, 1), dtype=torch.long)
            t_input["input_ids"] = torch.cat((t_input["input_ids"], eos), dim=1)
            t_input["attention_mask"] = torch.cat(
                (t_input["attention_mask"], eos_attn), dim=1
            )
        return t_input
...
def run_model_embedding(
    ...
    model = AutoModel.from_pretrained(
        model_path_or_name, config=model_config, torch_dtype=torch_dtype
    )
    ...
    test_dataset = EmbDataset(
        test_data, tokenizer, max_seq_len, args.has_template, qorp
    )
    ...
    with torch.no_grad():
        for data in dataloader:
            ...
            sentence_embeddings = sentence_embedding(
                ...
                data["attention_mask"],
                args.sentence_pooling_method,
                args.normlized,
            )

            sentence_embeddings = accelerator.gather_for_metrics(
                sentence_embeddings
            ).tolist()
