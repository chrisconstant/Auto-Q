...
class EmbedCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        if not self.has_template:
            ...
            eos = torch.full(
                (q_batch, 1), self.tokenizer.eos_token_id, dtype=torch.long
            )
            eos_attn = torch.ones((q_batch, 1), dtype=torch.long)
            q_collated["input_ids"] = torch.cat((q_collated["input_ids"], eos), dim=1)
            q_collated["attention_mask"] = torch.cat(
                (q_collated["attention_mask"], eos_attn), dim=1
            )

            d_batch = len(d_collated["input_ids"])
            eos = torch.full(
                (d_batch, 1), self.tokenizer.eos_token_id, dtype=torch.long
            )
            eos_attn = torch.ones((d_batch, 1), dtype=torch.long)
            d_collated["input_ids"] = torch.cat((d_collated["input_ids"], eos), dim=1)
            d_collated["attention_mask"] = torch.cat(
                (d_collated["attention_mask"], eos_attn), dim=1
            )
