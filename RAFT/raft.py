...
def get_chunks(file_path: str, doctype="pdf", chunk_size=512) -> list[str]:
    """
    ...
    if doctype == "api":
        ...
        client = Client(credentials=Credentials(api_key=api_key, api_endpoint=api_endpoint))
...
def generate_instructions_gen(chunk, x=5) -> list[str]:
    ...
    system_prompt = f"""
    <|system|>
    You are a synthetic question-answer pair generator. Given a chunk of context about some topic(s), 
    generate {x} example questions a user could ask and would be answered using information from the chunk. 
    For example, if the given context was a Wikipedia paragraph about the United States, an example question 
    could be 'How many states are in the United States?'
    ...
    response = next(iter(responses)).results[0].generated_text
    queries = response.split('\n')
    queries = [strip_str(q) for q in queries]
    queries = [q for q in queries if any(c.isalpha() for c in q)]
    # try to only get the last x lines which should be the questions
    # queries = queries[-x:]
    return queries 
